...
thread_yield (void) 
{
  ...
  ASSERT (!intr_context ()); // true if in a external intr
...
/* added code */
bool thread_cmp_waketicks(struct list_elem *first, struct list_elem *second, void* aux)
{
  struct thread *fthread = list_entry(first, struct thread, elem);
  struct thread *sthread = list_entry(second, struct thread,elem);

  return fthread->wake_ticks < sthread->wake_ticks;
}
